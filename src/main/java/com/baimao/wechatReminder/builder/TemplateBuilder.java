package com.baimao.wechatReminder.builder;

import com.baimao.wechatReminder.config.TemplateIdConfig;
import com.baimao.wechatReminder.config.ToUserInfoConfig;
import com.baimao.wechatReminder.entity.ParamDto;
import com.baimao.wechatReminder.entity.Weather;
import com.baimao.wechatReminder.entity.WechatSendBody;
import com.baimao.wechatReminder.service.SweetWordService;
import com.baimao.wechatReminder.service.WeatherService;
import com.baimao.wechatReminder.utils.DataUtil;
...
public class TemplateBuilder {
     ...
     */
    public WechatSendBody buildMorningTemplate(String openId) {
        Map data = new HashMap();
        WechatSendBody wechatSendBody = new WechatSendBody(openId, templateIdConfig.getMorning(), data);

        data.put("today", new ParamDto(DataUtil.getNowDayAndWeekGregorian(), "#cc33cc"));
        data.put("schoolDay", new ParamDto(DataUtil.getFutureDays(userInfoConfig.getSchoolDay()), "#ff3399"));
...
//        data.put("memorial", new ParamDto(DataUtil.getDiffDaysYears(userInfoConfig.getLoveDay()), "#66ff00"));
        ...
        if (DataUtil.isWorkDay() && (weather.getWea().contains("雨"))) {
            data.put("notice", new ParamDto(System.lineSeparator() + "今天有雨，记得带伞，注意安全，走路不要玩手机，回家记得报平安！", "#1cbbb4"));
        }
     ...
     * @return
     */
    public WechatSendBody buildEveningTemplate(String openId) {
        Map data = new HashMap();
        WechatSendBody wechatSendBody = new WechatSendBody(openId, templateIdConfig.getEvening(), data);
        data.put("today", new ParamDto(DataUtil.getNowDayAndWeekGregorian(), "#cc33cc"));
        data.put("word", new ParamDto(sweetWordService.getSweetWord(), "#8C8C8C"));
        Weather weather = weatherService.getWeather();
        if (DataUtil.isWorkDay() && (weather.getWea().contains("雨"))) {
     ...
     * @return
     */
    public WechatSendBody buildSnackTemplate(String openId) {
        Map data = new HashMap();
        WechatSendBody wechatSendBody = new WechatSendBody(openId, templateIdConfig.getNightSnackTime(), data);
        data.put("nightSnackTime",new ParamDto(DataUtil.getSnackTime(),"#ff3399"));
        return wechatSendBody;
    }
